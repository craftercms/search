...
public class ElasticsearchAdminServiceImpl implements ElasticsearchAdminService {
    ...
    @Override
    public void duplicateIndex(String srcAliasName, String destAliasName) throws ElasticsearchException {
        try {
            doCreateIndex(elasticsearchClient, destAliasName, indexNameSuffix,
                    getLocale(srcAliasName), true, doGetIndexSettings(elasticsearchClient, srcAliasName));
            doReindex(elasticsearchClient, srcAliasName, destAliasName);
        } catch (IOException e) {
            throw new ElasticsearchException(srcAliasName, format("Error duplicating index '%s'", srcAliasName), e);
        }
    }
